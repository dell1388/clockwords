> **Note** — this is the Word War 3 branch: a military reskin of the
> Clockwords remake. Everything below still describes the mechanics, which
> are unchanged; only names and art differ (see the README table).
...
- The magazine dispenses letters into **eight chambers**; letters you type that are
...
- Materials and their effects:
  ...
  - **Thermite** — sets tanks on fire, 20 damage over 4 seconds, 3 secrets. ²
  - **Amethyst** — 15 damage, passes through up to 2 targets, 4 secrets. ²
  - **Lazurite** — freezes tanks, stopping movement for 4 seconds, 5 secrets. ²
...
- **Transmute**: put two letters of the same value into the machine to get a
  stronger one. ⁴
- **Secrets** are the currency, spent between levels in the **armoury**, where
  you add letters the tanks dropped or transmute what you have. ³ ⁴
- The chambers recycle their letters cyclically, like reshuffling a deck. ²
- Enemies are mechanical **tanks** and **cockroaches** that cross the room to
  **steal your intel / the dossiers of your formula**; let them and you lose. ¹ ³
...
⁵ [Kongregate game dossier](https://www.kongregate.com/en/games/gabob/clockwords-prelude)
...
- **The crucible.** Any even number of letters of one level goes in at once and is
  ...
  level. **The firing range** — a sandbox of standing hulks with a live damage
...
- **Chambers unseal on a full house**: only a single word that spends every loaded
  ...
  tank can be clicked to swap its letter. The magazine also refuses to load duplicate
  letters while it has distinct ones left. The original ran all eight from the start.
- **Magazine and storage.** A 15–50 letter working magazine with everything else parked
...
- **The cannon** sits in a corner, fires one shell per letter at 300 rpm, holds
  ...
  on a tank that is as good as dead. The original fired faster.
- **Tank pathing.** One open door, and a single painted route: four sweeps across
...
- **Loot tables** weighted by wave number and by how tough the tank was, with the
  dropped letter rising out of the wreck and kept whether the wave is won or lost.
- **Score** as a pure tally: half a word's damage plus the square of its length,
  a bounty per tank, and a clear bonus scaled by surviving pages.
- **Exact numbers** for the long-word curve (`(length/3) ^ 1.61`, so double length
  ...
  Aetherium's chain.
- **Magazine overload** — the bonus for spending every loaded chamber in one word.
...
- **Tank roster.** Spiders and roaches are documented; ticks, the Ironclad Beetle,
...
- **Five dossiers** in a safe in the corner opposite the cannon, returning to it when
  you kill the carrier. The original kept them on the machine itself.
- **The night's work** — the ranked per-word damage ledger in the armoury.
- **The campaign.** Ten handmade waves, then a procedural curve that keeps going
  indefinitely with a boss on every tenth. One magazine carries the whole run and
  only ever changes in the armoury; failing a wave costs that wave only, and
